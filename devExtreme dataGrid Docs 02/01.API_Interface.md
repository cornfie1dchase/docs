...
type GroupingInterval = 'year' | 'quarter' | 'month' | 'day' | 'dayOfWeek' | 'hour' | 'minute' | 'second';
type KeySelector<T> = string | ((source: T) => string | number | Date | Object);

type GroupDescriptor<T> = KeySelector<T> | (OrderingDescriptor<T> & {
...
type SummaryDescriptor<T> = KeySelector<T> | SelectionDescriptor<T> & {
...
​	以上定义的接口字段，能够覆盖表格所有远程操作；**接口设计**时可以此为**基准**，**必须的属性名称及其作用不变**的情形下，可根据实际进行部分调整。
